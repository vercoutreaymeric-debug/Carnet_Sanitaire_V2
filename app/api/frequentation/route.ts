...
export async function GET(req: NextRequest) {
  ...
  else if (from || to) where.date = { gte: from ?? undefined, lte: to ?? undefined }

  const frequentations = await prisma.frequentation.findMany({ where, orderBy: { date: 'desc' } })
...
export async function POST(req: NextRequest) {
  ...
  const freq = await prisma.frequentation.create({
    data: { etablissementId: session.etablissementId, date: body.date, scolaire, club, publicCount, autre, total, reportEau, releveEau, totalEau, litresParBaigneur },
  })
